...
void BoardInit(void)
{
    ...
    */
    ...
    // CORE TIMER 1 ms
    // clang-format off
    int T = CORE_TIMER_INTERRUPT_TICKS;
    asm volatile("mtc0   $0,$9");
    asm volatile("mtc0   %0,$11" : "+r"(T));
    // clang-format on
    _CP0_SET_COMPARE(CORE_TIMER_INTERRUPT_TICKS);
    mCTSetIntPriority(4);
    mCTSetIntSubPriority(0);
    mCTIntEnable(1);

#if 0 // FREERTOS ?!?!?
    // Assign shadow register sets to interrupt priorities 1 through 7
    PRISS = 0x76543210;
